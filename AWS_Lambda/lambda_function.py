...
def AVR_BLE_DATA():
	...
	child.expect("\r\n", timeout=10)
	if child.before:
		lightData = "Light: " + str(int(child.before.replace(" ", ""),16))
		client.publish(topic='BLE/data', payload=lightData)

	child.sendline("char-read-hnd 0x75")
	child.expect("Characteristic value/descriptor: ", timeout=10)
	child.expect("\r\n", timeout=10)
	if child.before:
		temperatureData = "Temperature: " + str(int(child.before.replace(" ", ""),16))
